# -*- coding: utf-8 -*-
"""
Created on Mon Oct 31 16:45:50 2022
...
class DisGNN(SocialRecommender,GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(DisGNN, self).readConfiguration()
        args = config.OptionConf(self.config['DisGNN'])
    ...
    def trainModel(self):
        ...
        for i in range(self.n_layer):
            for j in range(self.n_channel):
                self.weights['w1%d_%d'%(i,j)] = tf.Variable(initializer([2 * self.emb_size, 2 * self.emb_size]), name='w1%d_%d'%(i,j))
                self.weights['w2%d_%d'%(i,j)] = tf.Variable(initializer([2 * self.emb_size, 1]), name='w2%d_%d'%(i,j))
        ...
        def loop_user(users):
            ...
            possibility = tf.nn.relu(possibility) #m*2d ---------shoule be leaky relu-----------------
            possibility = tf.matmul(possibility, self.weights['w2%d_%d'%(i,j)]) #m*1
        ...
        for i in range(self.n_layer):  
            for j in range(self.n_channel):
                ...
                channel_possibility = tf.nn.softmax(tf.nn.sigmoid(raw_possibility)) #m+*m
                #new_user_embeddings_c = tf.matmul(all_user_embeddings[i], self.weights['feat%d_%d'%(i,j)]) #m*d
                new_user_embeddings_c = tf.matmul(channel_possibility, all_user_embeddings[i]) #m+*d
                # semantic_embeddings = tf.concat([all_user_embeddings[i][self.segment_u:self.segment_u+100], new_user_embeddings_c], axis=1) #m+*2d
                # predicted = tf.nn.sigmoid(tf.matmul(semantic_embeddings, self.d_weights['discriminator1'])) #m+*2d
                # predicted = tf.nn.softmax(tf.matmul(predicted, self.d_weights['discriminator2'])) #m+*channel
                # loss_channel += tf.reduce_sum(predicted, axis=0)[j] 
        ...
        train_pre = opt.minimize(loss) #+loss_edge+loss_homo+loss_hetero+loss_channel
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(self.next_batch_pairwise()):
                ...
                _, l, self.U, self.V, all_weights = self.sess.run([train_pre, loss, user_embeddings, item_embeddings, self.weights],
        ...
        import pickle
        name = 'weight_folder/Precision'+'_'+str(self.bestPerformance[1]['Precision'])
        with open(name, 'wb') as fp:
            pickle.dump(all_weights, fp)
