...
class GraphRecommender(DeepRecommender):
    ...
    def create_joint_sparse_adjaceny(self):
        ...
        adj_mat = tmp_adj + tmp_adj.T #(u+i)*(u+i)
        #print(adj_mat.sum(1))
        # pre adjcency matrix
        rowsum = np.array(adj_mat.sum(1))
        d_inv = np.power(rowsum, -0.5).flatten()
        d_inv[np.isinf(d_inv)] = 0. #(u+i)
        d_mat_inv = sp.diags(d_inv) #(u+i)
        norm_adj_tmp = d_mat_inv.dot(adj_mat) #horizontal norm
        adj_matrix = norm_adj_tmp.dot(d_mat_inv) #vertical norm
        return adj_matrix
