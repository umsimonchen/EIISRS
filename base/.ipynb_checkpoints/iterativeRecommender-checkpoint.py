...
class IterativeRecommender(Recommender):
    ...
    def predictForRanking(self,u):
        ...
        if self.data.containsUser(u):
            return self.Q.dot(self.P[self.data.user[u]]) #only get the item-score of the user that has been trained 
        else:
    ...
    def isConverged(self,epoch):
        ...
        random.shuffle(self.data.trainingData)
        return converged
    ...
    def ranking_performance(self,epoch):
        ...
        for user in self.data.testSet_u:
            candidates = self.predictForRanking(user) #MHCN-length = n 
            #print(len(candidates))
            # predictedItems = denormalize(predictedItems, self.data.rScale[-1], self.data.rScale[0])
            ratedList, ratingList = self.data.userRated(user) #return the trainset_u u' bought'items--(key=item name,value=1.0)
            for item in ratedList:
                candidates[self.data.item[item]] = 0 #bought item set to zero to avoid its affect
            ids, scores = find_k_largest(N, candidates)
            item_names = [self.data.id2item[iid] for iid in ids] #return back the corresponding item name for evaluation
            recList[user] = list(zip(item_names, scores)) #after zipping the format would be multi-tuples
        measure = Measure.rankingMeasure(self.data.testSet_u, recList, [N]) #calculate the precision/recall/f1/ndcg
        try:
        ...
        if len(self.bestPerformance)>0:
            ...
            for k in self.bestPerformance[1]:
                if self.bestPerformance[1][k] > performance[k]: 
                    ...
                    count -=1
            self.count = count
            if count<0: #>= not account, meaning any 3 or 4 new best will update
                self.bestPerformance[1]=performance
                self.bestPerformance[0]=epoch+1
                self.saveModel() #MHCN
                if self.earlyStop < 50:
                    try:
                        self.no_improve[self.earlyStop] += 1
                    except:
                        pass
                self.earlyStop=0
            else:
